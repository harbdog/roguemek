...
<div class="fieldcontain ${hasErrors(bean: battleMechInstance, field: 'ownerPilot', 'error')} required">
	...
	<g:select id="ownerPilot" name="ownerPilot.id" from="${roguemek.model.Pilot.list()}" optionKey="id" required="" optionValue="${{it?.firstName+" \""+it?.ownerUser?.callsign+"\" "+it?.lastName}}" value="${battleMechInstance?.ownerPilot?.id}" class="many-to-one"/>
