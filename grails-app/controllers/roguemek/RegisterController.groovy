...
class RegisterController extends AbstractS2UiController {
	...
	def register(RegisterCommand command) {
		...
		def user = lookupUserClass().newInstance(callsign: command.callsign, username: command.username,
...
class RegisterCommand {
	...
	static constraints = {
		callsign blank: false, validator: { value, command ->
			if (value) {
...
class RegisterControllerMek extends grails.plugin.springsecurity.ui.RegisterController {
	static boolean checkPasswordMinLength(String password, command) {
		def conf = SpringSecurityUtils.securityConfig
